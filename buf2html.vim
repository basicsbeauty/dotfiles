...
function! s:BrightAverage(hexspec)
    ...
    let rv = printf( '%d', float2nr(( R + G + B ) / 3) )
    let factrRGB =
      \ [ printf('%02x', float2nr(bfact * (R || Rs)))
      \ , printf('%02x', float2nr(bfact * (G || Gs)))
      \ , printf('%02x', float2nr(bfact * (B || Bs))) ]
